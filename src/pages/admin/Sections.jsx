...
export default function AdminSections() {
  ...
  async function load() {
    setLoading(true)
    const [{ data: secs }, { data: tchs }] = await Promise.all([
      supabase.from('sections').select('*, teachers(teacher_name)').order('section_name'),
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editRow) {
        await supabase.from('sections').update(payload).eq('id', editRow.id)
  ...
  async function toggleActive(s) {
    await supabase.from('sections').update({ is_active: !s.is_active }).eq('id', s.id)
  ...
  async function deleteSection(s) {
    ...
    toast('Section deleted', 'success'); load()
  }
        ...
        {sections.map(s => (
          <div key={s.id} className="card flex flex-col gap-3">
            ...
            <div className="flex items-center gap-1 justify-end pt-1 border-t border-surface-border">
              <button className="btn-ghost p-1.5 text-xs" onClick={() => openEdit(s)}><Edit2 size={14} /></button>
