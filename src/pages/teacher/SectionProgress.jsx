      ...
      const secs = await dbQuery(
        supabase.from('sections')
          .select(`id, section_name,
            students(id, student_name, reg_number,
          ...
          <div className="space-y-2">
            {currentSection.students.map(stu => (
              <div key={stu.id} className="border border-surface-border rounded-xl overflow-hidden">
                <button className="w-full flex items-center gap-4 px-4 py-3 hover:bg-surface transition-colors text-left"
                    ...
                    {!stu.attempts.length
                      ...
                      : (
                        <table className="table-base">
                          <thead><tr><th>#</th><th>Date</th><th>Score</th><th>Correct</th><th>Time</th><th>Status</th></tr></thead>
                          <tbody>
                            {stu.attempts.map((a, i) => (
                              <tr key={a.id}>
                                ...
                                <td><span className={`badge ${a.status === 'completed' ? 'badge-success' : 'badge-accent'}`}>{a.status}</span></td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
            ...
            {!currentSection.students.length && <p className="text-ink-muted text-sm py-4">No students in this section.</p>}
          </div>
        </div>
      )}
    </TeacherLayout>
